...
Another issue i encountered was another limitation of materialise, this was to do with the javascript used in the application. As stated in script.js. When run through JSLint the javascript has 6 errors in the code. However due to limitations in materialize i am unable to change the code to prevent any errors when passed through jslint without hindering my application. I managed to remove these errors however the javascript elements in my application stopped working as a result. The javascript code is structured as instructed by materialize.css in order for all elements of the application using javascript to work correctly.

As stated above, and in testing section, in the instance where errors do occur i have implemented error handling events in routes.py that will handle internal server errors and 404 errors. Once the site has encountered these the app will direct the user to an errors page with a message to the user and a link back to the application without the use of the browsers navigation buttons. This has been my attempt at a resolution to these errors to improve the user experience as much as i can.
